...
/* Optional: only needed if you plan to use theme.mode */
@variant dark (.dark &);
```

Without this, `theme.mode: 'dark'` will still update the CSS custom properties (`--df-color-*`) correctly, but DayFlow's internal `dark:` utility rules will only activate when the system preference is dark.
