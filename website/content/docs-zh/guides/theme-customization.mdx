...
DayFlow 的组件内部仍会使用 Tailwind 工具类（如 `flex`、`bg-white`、`text-gray-600`）来处理布局和中性色样式，因此你的 Tailwind 实例仍需扫描 DayFlow 的文件来生成这些类。但旧的语义类（如 `bg-primary`、`text-primary`）已经不再是 DayFlow 内部的依赖。

当你导入 `@dayflow/core/dist/styles.components.css` 时，Tailwind 仍然需要扫描 DayFlow 已发布的 JavaScript 文件，才能发现库内部用到的工具类。Tailwind 的自动扫描不会包含 `node_modules`，所以即使使用 Vite + `@tailwindcss/vite`，这一步 `@source` 仍然是必需的。

请在 CSS 入口文件中添加 `@source` 指令：
...
@source '../node_modules/@dayflow/plugin-drag/dist/**/*.js'; /* 使用拖拽插件时 */
@source '../node_modules/@dayflow/plugin-sidebar/dist/**/*.js'; /* 使用侧边栏插件时 */
```

如果缺少此配置，Tailwind 将无法生成 DayFlow 组件内部大量依赖的工具类，界面可能会部分或完全失去样式。

如果你是在本地 DayFlow workspace 中联调源码，而不是使用已发布的 npm 包，那么请把 `@source` 指向对应 workspace 里的源码目录。
