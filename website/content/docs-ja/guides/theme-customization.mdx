...
DayFlow のコンポーネントは、レイアウトや中立色のために Tailwind ユーティリティクラス（`flex`、`bg-white`、`text-gray-600` など）を引き続き使用します。そのため、プロジェクト側の Tailwind インスタンスは DayFlow のファイルをスキャンしてこれらのクラスを生成する必要があります。一方で、`bg-primary` や `text-primary` のような旧来のセマンティック Tailwind 名は、もはや DayFlow 内部の依存ではありません。

`@dayflow/core/dist/styles.components.css` を読み込む場合でも、Tailwind は DayFlow の公開済み JavaScript ファイルをスキャンして、ライブラリ内部で使われているユーティリティクラスを検出する必要があります。Tailwind の自動ソース検出は `node_modules` をスキャンしないため、Vite + `@tailwindcss/vite` でもこの `@source` 設定は必要です。

CSS エントリーファイルに `@source` ディレクティブを追加してください。

```css
@import '@dayflow/core/dist/styles.components.css';
@import 'tailwindcss';

/* 公開済み npm パッケージを使う場合に必要 */
@source '../node_modules/@dayflow/core/dist/**/*.js';
@source '../node_modules/@dayflow/plugin-drag/dist/**/*.js'; /* ドラッグプラグイン使用時 */
@source '../node_modules/@dayflow/plugin-sidebar/dist/**/*.js'; /* サイドバープラグイン使用時 */
```

この設定がない場合、Tailwind は DayFlow コンポーネント内部で使われている多くのユーティリティクラスを生成できず、UI が部分的または全面的に崩れて見えることがあります。

公開済み npm パッケージではなく、ローカルの DayFlow workspace を相手に開発している場合は、`@source` をその workspace 内のソースディレクトリへ向けてください。
