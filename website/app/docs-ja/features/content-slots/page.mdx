...
| ジェネレーター名                  | 説明                                                               | 引数                                                            |
| :-------------------------------- | :----------------------------------------------------------------- | :-------------------------------------------------------------- |
| `eventContent`                    | イベントカードのカスタムレンダリング。ビューごとの上書きをサポート | `-`                                                             |
| `eventDetailContent`              | イベントクリック時に表示されるパネル/詳細コンテンツ。              | `{ event, position, onClose, onEventDelete, onEventUpdate }`    |
| `eventDetailDialog`               | イベントクリック時に表示されるカスタムダイアログ。                 | `{ event, isOpen, onClose, onEventDelete, onEventUpdate }`      |
| `colorPicker`                     | サイドバーで使用される小型のカラーピッカー。                       | `{ color, onChange, variant: 'sketch' }`                        |
| `createCalendarDialogColorPicker` | ダイアログで使用されるフル機能のカラーピッカー。                   | `{ color, onChange, onAccept, onCancel, variant: 'photoshop' }` |
