...
| 生成器名称                        | 描述                                           | 参数                                                            |
| :-------------------------------- | :--------------------------------------------- | :-------------------------------------------------------------- |
| `eventContent`                    | 日历事件卡片的自定义渲染。支持针对不同视图重写 | `-`                                                             |
| `eventDetailContent`              | 点击事件时弹出的面板/详情内容。                | `{ event, position, onClose, onEventDelete, onEventUpdate }`    |
| `eventDetailDialog`               | 点击事件时弹出的自定义对话框内容。             | `{ event, isOpen, onClose, onEventDelete, onEventUpdate }`      |
| `colorPicker`                     | 侧边栏中使用的小型颜色选择器。                 | `{ color, onChange, variant: 'sketch' }`                        |
| `createCalendarDialogColorPicker` | 对话框中使用功能完整的颜色选择器。             | `{ color, onChange, onAccept, onCancel, variant: 'photoshop' }` |
