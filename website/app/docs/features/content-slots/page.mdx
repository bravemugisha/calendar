...
| :-------------------------------- | :--------------------------------------------------------------- | :-------------------------------------------------------------- |
| `eventContent`                    | Custom rendering for event cards. Supports per-view overrides.   | `-`                                                             |
| `eventDetailContent`              | The content shown in the popover/panel when an event is clicked. | `{ event, position, onClose, onEventDelete, onEventUpdate }`    |
| `eventDetailDialog`               | Custom dialog shown when an event is clicked.                    | `{ event, isOpen, onClose, onEventDelete, onEventUpdate }`      |
| `colorPicker`                     | A small color picker used in the sidebar.                        | `{ color, onChange, variant: 'sketch' }`                        |
| `createCalendarDialogColorPicker` | A full-featured color picker used in dialogs.                    | `{ color, onChange, onAccept, onCancel, variant: 'photoshop' }` |
